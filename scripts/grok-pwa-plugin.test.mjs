...
test("injects project id on the script without visitor-facing grok/X meta", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "proj-123",
  });
...
test("does not emit grok:app_id", () => {
  ...
  assert.doesNotMatch(twice, /property="grok:app_id"/);
});
...
test("does not inject x:creator tags into visitor HTML", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "",
    creator: "@alice",
  ...
  assert.doesNotMatch(out, /property="x:creator"/);
  assert.doesNotMatch(out, /twitter:card/);
});
...
test("does not duplicate x:creator tags", () => {
  ...
  const twice = injectGrokPwaHead(once, ctx);
  assert.equal(once, twice);
  assert.equal(twice.split('property="x:creator" content=').length - 1, 0);
  assert.equal(twice.split('property="x:creator:id"').length - 1, 0);
});
...
test("does not duplicate twitter:card or og:title", () => {
  ...
  assert.equal(once, twice);
  assert.equal(twice.split('name="twitter:card"').length - 1, 0);
...
test("does not emit x:game:image for a public host when site.banner is set", () => {
  ...
  assert.doesNotMatch(out, /property="x:game:image"/);
  assert.doesNotMatch(out, /twitter:card/);
  assert.doesNotMatch(out, /content="old"/);
});
